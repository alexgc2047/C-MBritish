...
// article-submit.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_ARTICLE_SUBMIT):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
      $header_tags_array = tep_header_tag_page(FILENAME_ARTICLE_SUBMIT);
      WriteCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '');
    }
  break;
...
// article_manager_search_result.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_ARTICLE_MANAGER_SEARCH_RESULT):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
      $header_tags_array = tep_header_tag_page(FILENAME_ARTICLE_MANAGER_SEARCH_RESULT);
      WriteCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '');
    }
  break;

// article_manager_update_configure.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_ARTICLE_MANAGER_UPDATE_CONFIGURE):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
      $header_tags_array = tep_header_tag_page(FILENAME_ARTICLE_MANAGER_UPDATE_CONFIGURE);
      WriteCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '');
    }
  break;

// article_popup_print.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_ARTICLE_POPUP_PRINT):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
      $header_tags_array = tep_header_tag_page(FILENAME_ARTICLE_POPUP_PRINT);
      WriteCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '');
    }
  break;
...
// article_rss.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_ARTICLES_RSS):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
      $header_tags_array = tep_header_tag_page(FILENAME_ARTICLES_RSS);
      WriteCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '');
    }
  break;
...
// password_reset.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_PASSWORD_RESET):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
...
// pdfinvoice.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_CUSTOMER_PDF):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
