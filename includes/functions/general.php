...
// Customer Login at header
	function tep_customer_login() {
		...
		if (tep_session_is_registered('customer_first_name') && tep_session_is_registered('customer_id')) {
			$greeting_string = sprintf(TEXT_GREETING_PERSONAL_CUSTOMER, tep_output_string_protected($customer_first_name), tep_href_link(FILENAME_LOGOFF, '', 'SSL'));
	} else {
		$greeting_string = sprintf(TEXT_GREETING_GUEST_CUSTOMER, tep_href_link(FILENAME_LOGIN, '', 'SSL'), tep_href_link(FILENAME_CREATE_ACCOUNT, '', 'SSL'));
	}
