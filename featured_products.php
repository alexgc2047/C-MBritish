...
?>
	<div id="featured-products-container">
		<div id="featured-products-wrapper">
			<div id="featured-products-image" class="main" width="<?php echo SMALL_IMAGE_WIDTH + 10; ?>" ><?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $featured['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $featured['products_image'], $featured['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>'; ?></div>
			<div id="featured-products-info" class="main"><?php echo '<a class="link" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $featured['products_id']) . '">' . $featured['products_name'] . '</a><br /><br />' . TEXT_DATE_ADDED . ' ' . tep_date_long($featured['products_date_added']) . '<br /><br />' . TEXT_MANUFACTURER . ' ' . $featured['manufacturers_name'] . '<br /><br />' . TEXT_PRICE . ' ' . $products_price . '<br /><br />' ; ?></div>
			<div id="featured-products-button" class="smallText"><?php echo tep_draw_button(IMAGE_BUTTON_IN_CART, 'cart', tep_href_link(FILENAME_PRODUCT_INFO, tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $featured['products_id'])); ?></div>
		</div>
	 </div><hr>
<?php
    }
